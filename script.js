  ...
  const tampilData = () => {
    ...
    onSnapshot(q, (snapshot) => {
      ...
      snapshot.forEach(doc => {
        const item = doc.data();
        container.innerHTML += `
       <div class="bg-card p-3 rounded-2xl relative group transition-transform duration-200" style="border-radius: 5px; border-left: 4px solid #8b5cf6; box-shadow: 0px 1px 15px rgba(0,0,0,0.2);">
            ${!item.pinned ? `
...
<p style="margin-left: 10px; color: #222; font-family: 'calibri',sans-serif; font-size: 20px; line-height: 1.1; overflow-wrap: anywhere;">
